mudaeRanker.service("Characters", [
  "$http",
  "$interval",
  "$rootScope",
  "MergeCode",
    ...
    var service = {
      ...
      parseInputField: function (inputText) {
          ...
          try {
            if (mergeCharacters) {
              ...
              if (jsonInput.appState) {
                service.rankingInProgress =
                  jsonInput.appState.rankingInProgress;
            ...
            Utilities.showSuccess("Fin du traitement de l'entrée", true);
          } catch (e) {
            Utilities.showError(
              "Eh bien, vous avez foiré quelque chose : " + e.Message,
              true
            );
          }
        ...
        for (var i = 0; i < seriesLength; i++) {
          ...
          for (var j = 0; j < charactersLength; j++) {
            ...
            var originalName = characterString
              .replace(/ - https:.*/i, "")
              .replace(/(?: \| .*)?/gi, "")
              .trim();
            ...
            if (imageURLIndex > 0) {
              characterImage = characterString
                .substring(imageURLIndex + 3)
                .trim();
            }

            var characterName = originalName
              .replace(/(?: \([A-Z]+\))?/gi, "")
              .trim();
            ...
            if (lookupRequired) {
              // If a character was added, then add it to the series array, we'll have to look it up from Anilist
              series.characters.push(character);
            }
          }

          if (lookupRequired) {
            // If a character was added, the add the series to the seriesArray, since we'll have to look it up from Anilist
        ...
        if (seriesArray.length > 0) {
          // No reason to go to Anilist if there are no series to look up
          Utilities.showWarning("Recherche de personnages sur Anilist", true);
          service.anilistReqInterval = $interval(
            this.fetchSeries,
            800,
            0,
            true,
          ...
          service.anilistReqInterval.then(
            this.requestIntervalResolve,
            this.requestIntervalReject
          );
        }
      },
      ...
      requestIntervalReject: function () {
        Utilities.showSuccess(
          "Fini de chercher les personnages sur Anilist",
          true
        );
        console.log(
          "Intervalle rejeté (très probablement parce qu'il a été annulé, ce qui signifie que l'extraction est terminée)."
        );
      },
      ...
      fetchSeries: function (seriesArray) {
        ...
        $http
          .post(service.anilistApiUrl, queryBody, service.anilistConfig)
          .then(
            function (response) {
              ...
              for (var i = 0; i < characterCount; i++) {
                ...
                if (characterFirstName && characterFirstName.length) {
                  ...
                  if (characterFirstName.length) {
                    hasFirstName = true;
                    characterFirstName =
                      Utilities.minimizeName(characterFirstName);
                  }
                }
                ...
                if (characterLastName && characterLastName.length) {
                  ...
                  if (characterLastName.length) {
                    hasLastName = true;
                    characterLastName =
                      Utilities.minimizeName(characterLastName);
                  }
                }
                ...
                var characterNameUS =
                  (hasFirstName ? characterFirstName : "") +
                  (hasLastName ? characterLastName : "");
                var characterNameJP =
                  (hasLastName ? characterLastName : "") +
                  (hasFirstName ? characterFirstName : "");
                ...
                for (var j = 0; j < localCharactersLength; j++) {
                  ...
                  if (
                    localCharacterName === characterNameUS ||
                    localCharacterName === characterNameJP ||
                    localCharacterName === characterFirstName ||
                    localCharacterName === characterLastName
                  ) {
                    characterFound = true;
                  } else {
                    ...
                    for (var k = 0; k < alternativeNamesLength; k++) {
                      var alternativeName = Utilities.minimizeName(
                        alternativeNames[k]
                      );
              ...
              if (localCharactersLength > 0) {
                var hasAdditionalPages =
                  response.data.data.Media.characters.pageInfo.hasNextPage;
      ...
      getNextModeName: function () {
        switch (service.mode) {
          case Mode.Rank:
            return "Commencer la modification";
          case Mode.Edit:
          default:
            return "Commencer le classement";
        }
      },
      ...
      exportSort: function () {
        ...
        if (chars[0]["originalName"] === undefined) {
          Utilities.showError(
            "On dirait que vos personnages n'ont pas leurs noms d'origine enregistrés. Exécutez un autre $mmas et Parseer l'entrée pour qu'il puisse fusionner les informations appropriées, puis réessayez.",
            true
          );
          return;
        }
          ...
          if (total > 1) {
            ...
            for (var i = 1; i < total; i++) {
              if (i % 20 === 0) {
                output +=
                  "\n\n$sortmarry pos " +
                  chars[i - 1].originalName +
                  "$" +
                  chars[i].originalName;
              } else {
      ...
      mergeCharacter: function (character) {
        ...
        for (var i = 0; i < total; i++) {
          ...
          if (
            matchCharacter.minimizedName === character.minimizedName &&
            ...
            if (
              matchCharacter.imageUrl != null &&
              matchCharacter.imageUrl != ""
            ) {
      ...
      minimizeActiveCard: function () {
        if (service.mode === Mode.Edit) {
          if (service.activeIndex >= 0) {
            // If there is a currently active character
            ...
            service.characters[service.activeIndex].className = aClass.replace(
              /(?: )?CharacterFull( )?/,
              "$1"
            );
        ...
        return new Promise(function (resolve, reject) {
          if (service.mode === Mode.Edit) {
            if (service.activeIndex >= 0) {
              ...
              Utilities.confirm(
                "Vous êtes sûr de vouloir supprimer ce personnage ?",
                ...
                .fail(function (data, button) {
                  console.log(
                    'Alors pourquoi avez-vous cliqué sur le bouton "supprimer" ?'
                  );
                  service.inMessageBox = false;
                  reject();
                });
        ...
        if (service.mode === Mode.Edit) {
          if (index != service.activeIndex) {
            ...
            if (service.activeIndex >= 0) {
              ...
              // Remove the CharacterFull class from the currently active character
              var aClass = service.characters[service.activeIndex].className;
              service.characters[service.activeIndex].className =
                aClass.replace(/(?: )?CharacterFull( )?/, "$1");
            }
      ...
      skipLeft: function () {
        ...
        var skippedCharacter = service._rankedCharacters
          .splice(service._currentLeftIndex, 1)
          .pop();
      ...
      skipRight: function () {
        ...
        var skippedCharacter = service._rankedCharacters
          .splice(service._currentRightIndex, 1)
          .pop();
      ...
      presentCardsForComparison: function () {
        ...
        if (displayCards) {
          ...
          service.leftCompare =
            service._rankedCharacters[service._currentLeftIndex];
          service.rightCompare =
            service._rankedCharacters[service._currentRightIndex];
        } else {
      ...
      dragAndDropSortEnd: function (event) {
        if (service.rankingInProgress) {
          ...
          var currentInProgressIndex =
            PreferenceList.getIndexOfCurrentInProgress();
          ...
          var preferenceListNeedsUpdate =
            (characterWasOutside && characterIsNowInside) || characterWasInside;

          if (preferenceListNeedsUpdate) {
            if (characterWasInside) {
              if (characterIsNowInside) {
                if (currentInProgressIsInside) {
                  if (oldIndex === currentInProgressIndex) {
                    // If they moved the character that was in progress when they paused it
                    // Accept their decision and move on to the next character
                    PreferenceList.moveToNext();
                  } else if (oldIndex < currentInProgressIndex) {
                    // If they moved a character ranked higher than the character in progress when they paused it
                    if (newIndex >= currentInProgressIndex) {
                      // If the moved character is now ranked lower than the in progress character
                      PreferenceList.incrementCurrentInProgressRank();
                    }
                  } else if (oldIndex > currentInProgressIndex) {
                    // If they moved a character ranked lower than the character in progress when they paused it
                    if (newIndex <= currentInProgressIndex) {
                      // If the moved character is now ranked higher than the in progress character
              ...
              if (characterIsNowInside) {
                ...
                if (currentInProgressIsInside) {
                  if (newIndex <= currentInProgressIndex) {
                    // If they moved the character to a higher rank than the character in progress when they paused it
                    PreferenceList.decrementCurrentInProgressRank();
                  }
                } else {
      ...
      getDebugInfo: function (isRanking) {
        ...
        var outputString =
          ...
          PreferenceList.size +
          "\n\tcurrentIndex: " +
          PreferenceList.currentIndex +
          ": " +
          arrayToUse[PreferenceList.currentIndex].name +
          "\t\tLeft. C'est le personnage qui est placé. Note : Ceci n'est pas mis à jour avant la reprise." +
          "\n\tmin: " +
          PreferenceList.min +
          ": " +
          arrayToUse[PreferenceList.indices[PreferenceList.min].index].name +
          "\n\tcenterIndex: " +
          PreferenceList.centerIndex +
          ": " +
          arrayToUse[PreferenceList.indices[PreferenceList.centerIndex].index]
            .name +
          "\t\tRight.";
        ...
        if (PreferenceList.max < PreferenceList.indices.length) {
          outputString +=
            ...
            PreferenceList.max +
            ": " +
            arrayToUse[PreferenceList.indices[PreferenceList.max].index].name;
        } else {
          outputString +=
            "\n\tmax: " +
            PreferenceList.max +
            ": " +
            arrayToUse[PreferenceList.max].name;
        }

        outputString +=
          "\n\tlastCompare: " +
          PreferenceList.lastCompare +
          ": " +
          arrayToUse[PreferenceList.indices[PreferenceList.lastCompare].index]
            .name +
          "\t\tIl s'agit du dernier personnage auquel il est comparé à des fins de reprise." +
          "\n\tindices:";
