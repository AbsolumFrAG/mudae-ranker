mudaeRanker.service("PreferenceList", [
  "RankChoice",
  function (RankChoice) {
    var service = {
      size: 0,
      ...
      addAnswer: function (pref) {
        if (pref === RankChoice.Left) {
          // Left
          service.max = service.lastCompare = service.centerIndex;
        } else if (pref === RankChoice.Right) {
          // Right
      ...
      pause: function () {
        if (service.max !== service.indices.length || service.min !== 0) {
          // They made a choice on the current character
          // They made at least one choice, splice the character in at the last choice that was made
          service.indices.splice(service.lastCompare, 0, {
            index: service.currentIndex,
            skip: false,
          });
        }
      ...
      resume: function (count) {
        ...
        if (service.currentIndex < indicesLength) {
          // If the current index was added to the indices
          // Then we need to pop the index out and update it to its new value
          service.currentIndex = service.indices.splice(
            service.lastCompare,
            1
          )[0].index;
        }
      ...
      getQuestion: function () {
        if (service.currentIndex >= service.size)
          // If we've gone past the end of the array, stop
          return null;
      ...
      sortIndices: function () {
        ...
        for (
          var i = 0;
          i < indicesLength;
          i++ // Repopulate the array to the same size but now sorted as 0...n since those are the new indices
        ) {
      ...
      getIndexOfCurrentInProgress: function () {
        if (service.currentIndex < service.indices.length) {
          // If the current index was added to the indices
